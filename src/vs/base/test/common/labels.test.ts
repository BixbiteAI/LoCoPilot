...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'LoCoPilot', separator: { label: ' - ' } }), 'LoCoPilot');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'LoCoPilot', separator: { label: ' - ' } }), 'Untitled-1 - LoCoPilot');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'LoCoPilot', separator: { label: ' - ' } }), 'monaco - LoCoPilot');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'LoCoPilot', separator: { label: ' - ' } }), 'somefile.txt - monaco - LoCoPilot');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'LoCoPilot', separator: { label: ' - ' } }), '* somefile.txt - monaco - LoCoPilot');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'LoCoPilot', separator: { label: ' - ' } }), 'monaco - LoCoPilot');
	});
