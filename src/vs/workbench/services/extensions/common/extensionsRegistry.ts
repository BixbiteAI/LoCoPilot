	...
	properties: {
		engines: {
			...
			properties: {
				'vscode': {
					type: 'string',
					description: nls.localize('vscode.extension.engines.vscode', 'For LoCoPilot extensions, specifies the editor API version (vscode engine) that the extension is compatible with. Cannot be *. For example: ^0.10.5 indicates compatibility with a minimum version of 0.10.5.'),
					default: '^1.22.0',
				}
			}
		},
		publisher: {
			description: nls.localize('vscode.extension.publisher', 'The publisher of the LoCoPilot extension.'),
			type: 'string'
		},
		displayName: {
		...
		contributes: {
			description: nls.localize('vscode.extension.contributes', 'All contributions of the LoCoPilot extension represented by this package.'),
			type: 'object',
		...
		activationEvents: {
			description: nls.localize('vscode.extension.activationEvents', 'Activation events for the LoCoPilot extension.'),
			type: 'array',
			items: {
				type: 'string',
				defaultSnippets: [
					...
					{
						label: '*',
						description: nls.localize('vscode.extension.activationEvents.star', 'An activation event emitted on LoCoPilot startup. To ensure a great end user experience, please use this activation event in your extension only when no other activation events combination works in your use-case.'),
						body: '*'
					}
				],
			}
		...
		scripts: {
			...
			properties: {
				'vscode:prepublish': {
					description: nls.localize('vscode.extension.scripts.prepublish', 'Script executed before the package is published as a LoCoPilot extension.'),
					type: 'string'
				},
				'vscode:uninstall': {
					description: nls.localize('vscode.extension.scripts.uninstall', 'Uninstall hook for LoCoPilot extension. Script that gets executed when the extension is completely uninstalled from LoCoPilot which is when LoCoPilot is restarted (shutdown and start) after the extension is uninstalled. Only Node scripts are supported.'),
					type: 'string'
				}
			}
		},
