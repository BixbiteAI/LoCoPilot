...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[ChatConfiguration.LocopilotLlamaCppAutoSpeculative]: {
			type: 'boolean',
			markdownDescription: nls.localize('locopilot.llamaCpp.autoSpeculative.description', "Speed up local GGUF models with **automatic speculative decoding**. When a small same-family draft model is available for the selected model (downloaded alongside it), it fits your RAM, and a GPU/Metal backend is active, it is used to draft tokens the main model verifies in one pass (typically 1.5-2x faster generation). Skipped automatically for MTP models (they self-draft), when you set `#locopilot.llamaCpp.draftModelPath#` or `#locopilot.llamaCpp.promptLookup#` manually, and on llama.cpp builds that don't support speculation (detected at launch; the server is relaunched without it). N-gram drafting is no longer used as an automatic fallback - it did not draft anything in practice on real agent traffic - but it can still be enabled by hand via `#locopilot.llamaCpp.promptLookup#`."),
			default: true,
		},
