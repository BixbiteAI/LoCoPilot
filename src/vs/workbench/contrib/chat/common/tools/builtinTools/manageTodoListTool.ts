...
export function createManageTodoListToolData(): IToolData {
	...
	return {
		...
		modelDescription: 'Manage a structured todo list to track progress and plan tasks throughout your coding session.\n\nBefore calling this tool, apply this test: does the task need 3+ distinct non-trivial steps, or span multiple files? If not, do NOT use this tool - just do the work directly.\n\nDo NOT use for (do the work directly instead):\n- Single-file edits, renaming a symbol, adding one import, fixing a typo\n- Explaining code, answering a question, purely conversational/informational requests\n- Reading files or performing simple searches\n\nUse this tool when:\n- Complex multi-step work requiring planning and tracking\n- The user provides multiple tasks or requests (numbered/comma-separated)\n- New instructions that genuinely require multiple steps\n- BEFORE starting work on any todo (mark as in-progress)\n- IMMEDIATELY after completing each todo (mark completed individually)\n\nCRITICAL workflow:\n1. Plan tasks by writing todo list with specific, actionable items\n2. Mark ONE todo as in-progress before starting work\n3. Complete the work for that specific todo\n4. Mark that todo as completed IMMEDIATELY\n5. Move to next todo and repeat\n\nTodo states:\n- not-started: Todo not yet begun\n- in-progress: Currently working (limit ONE at a time)\n- completed: Finished successfully\n\nOperations:\n- write (default): Provide the COMPLETE todoList. Each item needs a unique id, a title, a status, and an optional one-line description capturing intent/acceptance criteria. Keep exactly ONE item in-progress.\n- read: Omit todoList to retrieve the current list (useful to re-orient after a long stretch of work).\n\nIMPORTANT: Mark todos completed as soon as they are done. Do not batch completions. Keep ids unique and exactly one todo in-progress.',
...
export class ManageTodoListTool extends Disposable implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: any, _progress: any, _token: CancellationToken): Promise<IToolResult> {
		...
		if (!chatSessionResource) {
			return {
				content: [{
					kind: 'text',
					value: 'Error: No session resource available. Next: Ensure the todo tool is called from an active chat session.'
				}],
				toolResultError: 'No session resource'
			};
		}
		...
		try {
			...
			return {
				content: [{
					kind: 'text',
					value: `Error: ${errorMessage}. Next: Retry with a valid todoList (id, title, status for each item), or continue with other tools.`
				}],
	...
	private handleRead(todoItems: IChatTodo[], sessionResource: URI): string {
		if (todoItems.length === 0) {
			return 'No todo list found. Proceed to the next step or goal.';
		}
	...
	private handleWriteOperation(args: IManageTodoListToolInputParams, chatSessionResource: URI): IToolResult {
		if (!args.todoList) {
			return {
				content: [{
					kind: 'text',
					value: 'Error: todoList is required for write operation. Next: Provide a todoList array with id, title, and status for each item.'
				}],
				toolResultError: 'Missing todoList'
			};
		}
		...
		return {
			content: [{
				kind: 'text',
				value: `Successfully wrote todo list. Proceed to the next step or goal.${warnings.length ? '\n\n' + warnings.join('\n') : ''}`
			}],
			toolMetadata: {
				warnings: warnings
			}
