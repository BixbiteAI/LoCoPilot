...
export class ChatResponseModel extends Disposable implements IChatResponseModel {
	...
	updateContent(responsePart: IChatProgressResponseContent | IChatTextEdit | IChatNotebookEdit, quiet?: boolean) {
		...
		// Always fire for tool invocations so the chat panel UI re-renders and shows tool call/result (renderer depends on onDidChange -> dataId -> refresh).
		if (responsePart.kind === 'toolInvocation' || responsePart.kind === 'toolInvocationSerialized') {
			this._onDidChange.fire(defaultChatResponseModelChangeReason);
		}
	}
...
export class ChatModel extends Disposable implements IChatModel {
	static getDefaultTitle(requests: (ISerializableChatRequestData | IChatRequestModel)[]): string {
		...
		// Rule-based title (no model call). Previously this was the raw first line of the message,
		// which is noisy for long/pasted asks; the heuristic classifies the intent and keeps the
		// distinguishing words. See chatTitleHeuristics.ts.
		return generateHeuristicChatTitle(message);
	}
	...
	acceptResponseProgress(request: ChatRequestModel, progress: IChatProgress, quiet?: boolean): void {
		...
		if (progress.kind === 'usedContext' || progress.kind === 'reference') {
			...
			if (progress.kind === 'toolInvocation' || progress.kind === 'toolInvocationSerialized') {
				const partsLength = request.response?.response?.value?.length ?? 0;
				this.logService.info(`[LoCoPilot Tool UI] ChatModel.acceptResponseProgress: requestId=${request.id}, adding toolInvocation, response.value.length before=${partsLength}`);
			}
			request.response.updateContent(progress, quiet);
		}
	}
